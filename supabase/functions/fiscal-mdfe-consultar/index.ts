// Edge Function: fiscal-mdfe-consultar
//
// Consulta a situação de um MDF-e diretamente na SEFAZ (útil quando o status
...
Deno.serve(async (req) => {
  try {
    ...
    const wrapperResp = await fetch(wrapperUrl(`/mdfe/${empresaId}/${chaveAcesso}/consultar`), {
      method: "GET",
    ...
    if (!wrapperResp.ok || !resultado.ok) {
      return jsonResponse({ ok: false, erro: resultado.erro ?? "Falha ao consultar MDF-e" }, 502);
    }
