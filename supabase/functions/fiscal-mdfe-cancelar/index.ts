// Edge Function: fiscal-mdfe-cancelar
//
...
Deno.serve(async (req) => {
  try {
    ...
    if (doc.status !== "autorizado") {
      return jsonResponse({ ok: false, erro: `Só é possível cancelar um MDF-e autorizado (status atual: ${doc.status})` }, 400);
    }
    ...
    const wrapperResp = await fetch(wrapperUrl(`/mdfe/${empresaId}/${chaveAcesso}/cancelar`), {
      method: "POST",
    ...
    if (!wrapperResp.ok || !resultado.ok) {
      return jsonResponse({ ok: false, erro: resultado.erro ?? "Falha ao cancelar MDF-e" }, 502);
    }

    await service.from("mdfe_documentos").update({ status: "cancelado" }).eq("id", doc.id);
