...
class _MasterDashboardPageState extends State<MasterDashboardPage> {
  ...
  Future<void> _loadAll() async {
    try {
      ...
      for (final c in checklists.take(3)) {
        ...
        activities.add(_ActivityItem(
          hora, Icons.task_alt_rounded, const Color(0xFF22C55E),
  ...
  Widget _buildSidebar(AppAuthProvider auth) {
    ...
    final items = <(IconData, String, _Sec, VoidCallback?)>[
      ...
      (Icons.domain_rounded, 'Empresas', _Sec.empresas, () => _showEmpresasDialog()),
      (Icons.supervisor_account_rounded, 'Usuários', _Sec.usuarios, () => nav(const AdminUsuariosPage())),
      ...
      (Icons.task_alt_rounded, 'Checklists', _Sec.checklists, () => nav(const HistoricoChecklistPage())),
      (Icons.bar_chart_rounded, 'Relatórios', _Sec.relatorios, () => nav(const RelatoriosPage())),
      (Icons.payments_rounded, 'Financeiro', _Sec.financeiro, () => nav(const ListaAbastecimentosPage())),
  ...
  List<_KpiData> _buildKpiList() {
    ...
    return [
      _KpiData(label: 'Empresas', value: '$_totalEmpresas', icon: Icons.domain_rounded,
          color: const Color(0xFFF59E0B), trend: trend(_tendEmpresas), trendUp: _tendEmpresas >= 0, spark: _sparkEmpresas),
      _KpiData(label: 'Usuários', value: _fmtNum(_totalUsuarios), icon: Icons.supervisor_account_rounded,
      ...
      _KpiData(label: 'Online Agora', value: '$_empresasOnline', icon: Icons.supervisor_account_rounded,
      ...
      _KpiData(label: 'Novas Empresas', value: '$_novasEmpresas', icon: Icons.storefront_rounded,
      ...
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                _acaoBtn(Icons.storefront_rounded, const Color(0xFF3B82F6), 'Nova Empresa', _showNovaEmpresaDialog),
                const SizedBox(width: 10),
                ...
                const SizedBox(width: 10),
                _acaoBtn(Icons.article_rounded, const Color(0xFFF59E0B), 'Logs do Sistema', _showLogsDialog),
                const SizedBox(width: 10),
    ...
    try {
      ...
      for (final e in (emp as List)) {
        results.add(_SearchResult('empresa', (e['nome'] ?? '').toString(),
            (e['status'] ?? '').toString(), Icons.domain_rounded, const Color(0xFF3B82F6)));
      }
