...
class _MasterDashboardPageState extends State<MasterDashboardPage> {
  ...
  Future<void> _loadAll() async {
    try {
      ...
      for (final c in checklists.take(3)) {
        ...
        activities.add(_ActivityItem(
          hora, Icons.local_library_rounded, const Color(0xFF22C55E),
  ...
  Widget _buildSidebar(AppAuthProvider auth) {
    ...
    final items = <(IconData, String, _Sec, VoidCallback?)>[
      ...
      (Icons.local_mall_rounded, 'Empresas', _Sec.empresas, () => _showEmpresasDialog()),
      (Icons.person_outline_rounded, 'Usuários', _Sec.usuarios, () => nav(const AdminUsuariosPage())),
      ...
      (Icons.local_library_rounded, 'Checklists', _Sec.checklists, () => nav(const HistoricoChecklistPage())),
      (Icons.bar_chart_rounded, 'Relatórios', _Sec.relatorios, () => nav(const RelatoriosPage())),
      (Icons.loyalty_rounded, 'Financeiro', _Sec.financeiro, () => nav(const ListaAbastecimentosPage())),
  ...
  List<_KpiData> _buildKpiList() {
    ...
    return [
      _KpiData(label: 'Empresas', value: '$_totalEmpresas', icon: Icons.local_mall_rounded,
          color: const Color(0xFFF59E0B), trend: trend(_tendEmpresas), trendUp: _tendEmpresas >= 0, spark: _sparkEmpresas),
      _KpiData(label: 'Usuários', value: _fmtNum(_totalUsuarios), icon: Icons.person_outline_rounded,
      ...
      _KpiData(label: 'Motoristas', value: _fmtNum(_totalMotoristas), icon: Icons.manage_accounts_rounded,
      ...
      _KpiData(label: 'Online Agora', value: '$_empresasOnline', icon: Icons.person_outline_rounded,
      ...
      _KpiData(label: 'Novas Empresas', value: '$_novasEmpresas', icon: Icons.local_offer_rounded,
  ...
  Widget _buildResumoAlertas() {
    ...
    final alertas = <(_AlertItem, VoidCallback)>[
      ...
      (_AlertItem(Icons.lock_rounded, const Color(0xFFEF4444), 'Mensalidades atrasadas', _mensalidadesAtrasadas),
          () => _showEmpresasDialog()),
      ...
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                _acaoBtn(Icons.local_offer_rounded, const Color(0xFF3B82F6), 'Nova Empresa', _showNovaEmpresaDialog),
                const SizedBox(width: 10),
                ...
                const SizedBox(width: 10),
                _acaoBtn(Icons.manage_accounts_rounded, const Color(0xFFEC4899), 'Novo Motorista',
                ...
                const SizedBox(width: 10),
                _acaoBtn(Icons.subject_rounded, const Color(0xFFF59E0B), 'Logs do Sistema', _showLogsDialog),
                const SizedBox(width: 10),
    ...
    try {
      ...
      for (final e in (emp as List)) {
        results.add(_SearchResult('empresa', (e['nome'] ?? '').toString(),
            (e['status'] ?? '').toString(), Icons.local_mall_rounded, const Color(0xFF3B82F6)));
      }
      ...
      for (final m in (mot as List)) {
        ...
        results.add(_SearchResult('motorista', (m['name'] ?? '').toString(),
            subtitle, Icons.manage_accounts_rounded, const Color(0xFFEC4899)));
      }
    } catch (_) {}
