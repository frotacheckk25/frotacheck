...
class _MasterDashboardPageState extends State<MasterDashboardPage> {
  ...
  Future<void> _loadAll() async {
    try {
      ...
      for (final c in checklists.take(3)) {
        ...
        activities.add(_ActivityItem(
          hora, Icons.done_all, const Color(0xFF22C55E),
  ...
  Widget _buildSidebar(AppAuthProvider auth) {
    ...
    final items = <(IconData, String, _Sec, VoidCallback?)>[
      ...
      (Icons.business, 'Empresas', _Sec.empresas, () => _showEmpresasDialog()),
      (Icons.group, 'Usuários', _Sec.usuarios, () => nav(const AdminUsuariosPage())),
      (Icons.directions_car_rounded, 'Veículos', _Sec.veiculos, () => nav(const VeiculosPage())),
      (Icons.person, 'Motoristas', _Sec.motoristas, () => nav(const MotoristasPage())),
      ...
      (Icons.done_all, 'Checklists', _Sec.checklists, () => nav(const HistoricoChecklistPage())),
      (Icons.bar_chart_rounded, 'Relatórios', _Sec.relatorios, () => nav(const RelatoriosPage())),
      (Icons.account_balance_wallet, 'Financeiro', _Sec.financeiro, () => nav(const ListaAbastecimentosPage())),
  ...
  List<_KpiData> _buildKpiList() {
    ...
    return [
      _KpiData(label: 'Empresas', value: '$_totalEmpresas', icon: Icons.business,
          color: const Color(0xFFF59E0B), trend: trend(_tendEmpresas), trendUp: _tendEmpresas >= 0, spark: _sparkEmpresas),
      _KpiData(label: 'Usuários', value: _fmtNum(_totalUsuarios), icon: Icons.group,
      ...
      _KpiData(label: 'Motoristas', value: _fmtNum(_totalMotoristas), icon: Icons.person,
      ...
      _KpiData(label: 'Online Agora', value: '$_empresasOnline', icon: Icons.group,
          color: const Color(0xFF10B981), spark: _sparkOnline),
      _KpiData(label: 'Receita Mês', value: 'R\$ ${_fmtMoney(_receitaMes)}', icon: Icons.attach_money,
          color: const Color(0xFF22C55E), trend: trend(_tendReceita), trendUp: _tendReceita >= 0, spark: _sparkReceita),
      _KpiData(label: 'Novas Empresas', value: '$_novasEmpresas', icon: Icons.store,
  ...
  Widget _buildResumoAlertas() {
    ...
    final alertas = <(_AlertItem, VoidCallback)>[
      ...
      (_AlertItem(Icons.person, const Color(0xFFF59E0B), 'CNHs vencendo', _cnhsVencendo),
          () => go(const MotoristasPage())),
      (_AlertItem(Icons.receipt, const Color(0xFFEF4444), 'Mensalidades atrasadas', _mensalidadesAtrasadas),
          () => _showEmpresasDialog()),
      ...
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                _acaoBtn(Icons.store, const Color(0xFF3B82F6), 'Nova Empresa', _showNovaEmpresaDialog),
                const SizedBox(width: 10),
                ...
                const SizedBox(width: 10),
                _acaoBtn(Icons.person, const Color(0xFFEC4899), 'Novo Motorista',
                ...
                const SizedBox(width: 10),
                _acaoBtn(Icons.format_list_bulleted, const Color(0xFFF59E0B), 'Logs do Sistema', _showLogsDialog),
                const SizedBox(width: 10),
    ...
    try {
      ...
      for (final e in (emp as List)) {
        results.add(_SearchResult('empresa', (e['nome'] ?? '').toString(),
            (e['status'] ?? '').toString(), Icons.business, const Color(0xFF3B82F6)));
      }
      ...
      for (final m in (mot as List)) {
        ...
        results.add(_SearchResult('motorista', (m['name'] ?? '').toString(),
            subtitle, Icons.person, const Color(0xFFEC4899)));
      }
    } catch (_) {}
